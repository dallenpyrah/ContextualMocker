...
public class ContextScope implements AutoCloseable {
     ...
     */
    public <T, R> ContextualMocker.OngoingContextualStubbing<T, R> when(T mock, Supplier<R> methodCall) {
        checkNotClosed();
     ...
     * @throws IllegalStateException if this scope has been closed.
     */
    public <T> void verify(T mock, ContextualMocker.ContextualVerificationMode mode, Supplier<?> methodCall) {
        checkNotClosed();
